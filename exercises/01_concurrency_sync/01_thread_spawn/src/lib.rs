...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let join_handle = thread::spawn(|| numbers.into_iter().map(|v| v * 2).collect());
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let t1 = thread::spawn(|| a.into_iter().sum());
    let t2 = thread::spawn(|| b.into_iter().sum());
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    thread::Builder::new().name(String::from("sleeper")).spawn(move || {
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s| {
        let handle1 = s.spawn(|| a.into_iter().sum());
        let handle2 = s.spawn(|| b.into_iter().sum());
