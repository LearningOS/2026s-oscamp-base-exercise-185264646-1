...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    let mut cmd = Command::new("cat")
        ...
        .spawn()
        .unwrap();
    cmd.stdin.take().unwrap().write_all(input.as_bytes()).unwrap();
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
    ...
    let output = Command::new(program)
        .args(args)
        .output()?;
    String::from_utf8(output.stdout).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, "Output can not convert to utf-8 string"))
}
