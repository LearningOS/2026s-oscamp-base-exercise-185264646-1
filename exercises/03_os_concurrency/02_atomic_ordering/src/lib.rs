...
impl OnceCell {
    ...
    pub fn init(&self, val: u32) -> bool {
        ...
        loop {
            let init = self.initialized.load(Ordering::Acquire);
            ...
            match self.initialized.compare_exchange(init, true, Ordering::AcqRel, Ordering::Acquire) {
                Err(_) => (),
                Ok(_) => {
                    break true;
                }
    ...
    pub fn get(&self) -> Option<u32> {
        ...
        match self.initialized.load(Ordering::Acquire) {
            true => Some(self.value.load(Ordering::Relaxed)),
            false => None,
        }
    }
}
