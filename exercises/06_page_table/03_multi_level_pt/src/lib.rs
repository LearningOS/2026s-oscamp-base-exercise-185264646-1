...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let vpn = Self::extract_vpn(va, 2);
        // root page directory is bound to exist
        let mut pte = self.nodes.get(&self.root_ppn).unwrap().entries[vpn];
        if pte & PTE_V == 0 {
            let ppn = self.alloc_node();
            pte = (ppn << PPN_SHIFT) | PTE_V;
            self.nodes.get_mut(&self.root_ppn).unwrap().entries[vpn] = pte;
        }

        let vpn = Self::extract_vpn(va, 1);
        let ppn = pte >> PPN_SHIFT;
        pte = self.nodes.get(&ppn).unwrap().entries[vpn];
        if pte & PTE_V == 0 {
            let next_ppn = self.alloc_node();
            pte = (next_ppn << PPN_SHIFT) | PTE_V;
            self.nodes.get_mut(&ppn).unwrap().entries[vpn] = pte;
        }

        let vpn = Self::extract_vpn(va, 0);
        let ppn = pte >> PPN_SHIFT;
        self.nodes.get_mut(&ppn).unwrap().entries[vpn] = pa | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for i in 0..=2 {
            let vpn = Self::extract_vpn(va, 2 - i);
            if let Some(pte) = self.nodes.get(&ppn) {
                let entry = pte.entries[vpn];
                if entry & PTE_V == 0 {
                    break;
                }
                ...
                if entry & (PTE_R | PTE_W | PTE_X) != 0 {
                    return TranslateResult::Ok((ppn << PPN_SHIFT) | (va & ((1 << (12 + (2 - i) * 9)) - 1) as u64));
                }
            } else {
                break;
            }
