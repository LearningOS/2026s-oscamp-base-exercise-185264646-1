...
impl FdTable {
    ...
    pub fn alloc(&mut self, file: Arc<dyn File>) -> usize {
        let index = match self.tbl.iter().position(|elem| elem.is_none()) {
            Some(index) => index,
            None => {
                self.tbl.push(None);
                self.tbl.len() - 1
    ...
    pub fn count(&self) -> usize {
        self.tbl.iter().filter(|e| e.is_some()).count()
    }
}
